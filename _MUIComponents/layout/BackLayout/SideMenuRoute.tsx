...
import { StyledListItemAvatar } from '@styles/styles_back_layout/mainLayoutStyle'
import { THEME_COLOR } from '@theme/colorManager'
import { generateHref } from '@utils/utilsFunction'

const SideMenuRoute = () => {
  const { getBackMenuList } = useAdmin()
  const { tokenState, backLeftSideMenuValue } = useMiddleWare()
  // 多國語系
  ...
  const getBackMenuLists = async () => await getBackMenuList()
  ...
  useEffectOnce(() => {
    ...
    if (tokenState) getBackMenuLists()
    produce((draft) => {
  ...
  return (
    ...
    >
      {backLeftSideMenuValue.map((route: { name: any; list: any }, idx: number) => (
        <div key={idx}>
          <ListItemText
            sx={{
              height: '2.5rem',
              // 左側選單大分類 => XX相關
              background: THEME_COLOR.SIDE_MENU.GROUP_BG_ADMIN,
              color: THEME_COLOR.SIDE_MENU.GROUP_TEXT_ADMIN,
              borderRadius: '1rem',
              py: 1,
              m: 1,
              textAlign: 'center',
          ...
          {route.list.map(
            (
              ...
              const checkPath = pathName === `/${list.router}`
