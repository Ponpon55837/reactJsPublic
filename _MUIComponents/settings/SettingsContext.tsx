...
export function SettingsProvider({ children }: SettingsProviderProps) {
  ...
  const langStorage = storageAvailable
    ? localStorage.getItem('i18nextLng')?.replace('-', '_')?.replace('-', '_')
    : ''
...
function getCookie(name: string) {
  if (typeof document === 'undefined') {
    throw new Error('getCookie() is not supported on the server. ')
  }
...
function setCookie(name: string, value: string, expireDays = 3) {
  const date = new Date()
